...
import { angleRightIcon, answerSelectIcon, correctResultIcon, incorrectResultIcon, restartIcon, clockIcon, angleRightWhiteIcon, takeTest, selectedAnswerIcon } from '../../public/icons';
...
const FullTestDetailBody: NextPage<PropType> = (props) => {
	...
	const handleResultIcon = (index: number) => {
		if (numberSelectedAnswer === -1) {
			...
			if (!isSubmit) {
				return <div>{selectedAnswerIcon}</div>;
			}
	...
	const handleSelectedAnswer = (index: number) => {
		if (numberSelectedAnswer !== -1 && isSubmit) return;
					...
					<div className={styles.questionPalette}>
						...
						<div className={styles.questionPaletteBody}>
							...
							{isSubmit && (
								<div className={styles.questionsStat}>
									<div className={styles.questionsStatItem}>
										...
										<div className={styles.questionsStatItemText}>{totalCorrectAnswers}/{questionList.length} Correct</div>
									</div>
									<div className={styles.questionsStatItem}>
										<div className={styles.redSquare} />
										<div className={styles.questionsStatItemText}>{totalIncorrectAnswers}/{questionList.length} Incorrect</div>
									</div>
								</div>
							)}
							<div className="mt-4" />
							<div className={styles.questionsProgress}>
								...
								<div className={styles.progressTitle}>0/{questionList.length}</div>
							</div>
							<div className={styles.questionPaletteFooter}>
								<div className={styles.btnRestart} onClick={() => handleSubmit()} role="presentation">
									Submit
								</div>
							</div>
						</div>
						...
						<>
							<div className="d-flex align-items-center justify-content-center gap-3 mb-3">
								...
								<div className="d-flex">
									<div className={styles.time}>120:00</div>
								</div>
							</div>
							<div className="text-center">{takeTest}</div>
							<div className={styles.takeTest} onClick={() => setIsTest(true)} role="presentation">Take test</div>
						</>
					) : (
						<>
							<div className={styles.questionView}>
								...
								<div className={styles.quizChoices}>
									...
									{currentQuestion?.rightAnswer === 0 && numberSelectedAnswer !== -1 && isSubmit && (
									...
									{currentQuestion?.rightAnswer === 1 && numberSelectedAnswer !== -1 && isSubmit && (
										handleShowExplanationElement()
									)}
									...
									{currentQuestion?.rightAnswer === 2 && numberSelectedAnswer !== -1 && isSubmit && (
										handleShowExplanationElement()
									)}
									...
									{currentQuestion?.rightAnswer === 3 && numberSelectedAnswer !== -1 && isSubmit && (
										handleShowExplanationElement()
									)}
								</div>
							</div>
...
export default FullTestDetailBody;